...
export function loadConfig() {
  ...
  const result = { default: null, environments: {} };
  if (user) {
    result.default = user.default || null;
    if (user.environments) Object.assign(result.environments, user.environments);
  }
  if (project) {
    if (project.default) result.default = project.default;
    if (project.environments) Object.assign(result.environments, project.environments);
  }
  return result;
}
...
export function resolveEnv(envName) {
  ...
  if (envName) {
    if (!envs[envName]) return dieMsg(`环境 "${envName}" 未配置。可用: ${keys.join(', ')}`);
    return { config: envs[envName], configFile: config, envName };
  }

  // auto-select
  if (keys.length === 0) return dieMsg('未配置任何环境。请先执行 "apollo-cli env add <name> --base-url <url>"');
  const picked = config.default || keys[0];
  if (!envs[picked]) return dieMsg(`默认环境 "${picked}" 不存在`);
  return { config: envs[picked], configFile: config, envName: picked };
}

export function getAllEnvs() {
  const config = loadConfig();
  return { envs: config.environments, default: config.default };
}

export function saveUserConfig(data) {
  const existing = readJSON(USER_CONFIG) || { environments: {} };
  const merged = { ...existing.environments, ...data.environments };
...
export function removeEnv(name) {
  const scopes = [];
  const user = readJSON(USER_CONFIG);
  if (user && user.environments && user.environments[name]) {
    delete user.environments[name];
  ...
  if (existsSync(PROJECT_CONFIG)) {
    ...
    if (project && project.environments && project.environments[name]) {
      delete project.environments[name];
...
/**
 * Set default env in the file that controls the effective default:
 * project config wins when it declares a default, else user config.
 */
export function setDefaultEnv(name) {
  const project = existsSync(PROJECT_CONFIG) ? readJSON(PROJECT_CONFIG) : null;
  ...
  const user = readJSON(USER_CONFIG) || { environments: {} };
  user.default = name;
  writeJSON(USER_CONFIG, user);
...
export function saveSession(envName, data) {
...
export function clearSession(envName) {
  ...
  if (!Object.hasOwn(all, envName)) return;
  delete all[envName];
  atomicWrite(SESSION_FILE, JSON.stringify(all, null, 2) + '\n', { encoding: 'utf8', mode: 0o600 });
}
