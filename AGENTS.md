...
- 直接运行：`node bin/apollo-cli.js --help`（或 `npm link` 后用 `apollo-cli`）。
- 测试：`npm test`（即 `node --test "test/**/*.test.js"`）；单文件 `node --test test/store.test.js`；调试单文件可直接 `node test/store.test.js`（TAP 直出）。不要用 `node --test test/`，目录参数会被当成文件报错。
...
- 当前零依赖，仅用 Node 内置模块。新增依赖需说明理由，能用内置模块实现就不要引入依赖。
...
- 只用 Node 内置 `node:test` + `node:assert/strict`，保持零依赖。测试放 `test/`，命名 `*.test.js`；`test/helpers.js` 是共享工具（顶层零副作用、不 import src）。
