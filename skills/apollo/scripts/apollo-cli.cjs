...
var require_identity = __commonJS({
  "node_modules/.pnpm/yaml@2.9.1/node_modules/yaml/dist/nodes/identity.js"(exports2) {
    ...
    var hasAnchor = (node) => (isScalar(node) || isCollection2(node)) && !!node.anchor;
...
var USER_DIR = (0, import_node_path.resolve)((0, import_node_os.homedir)(), ".apollo-cli");
var USER_CONFIG = (0, import_node_path.resolve)(USER_DIR, "config.json");
var SESSION_FILE = (0, import_node_path.resolve)(USER_DIR, "session.json");
var PROJECT_CONFIG = "apollo-cli.config.json";
function ensureDir(path) {
  if (!(0, import_node_fs2.existsSync)(path)) (0, import_node_fs2.mkdirSync)(path, { recursive: true });
}
...
function atomicWrite(path, content, options = "utf8") {
  ...
  (0, import_node_fs2.writeFileSync)(tmp, content, options);
  for (let i = 0; ; i++) {
    try {
  ...
  (0, import_node_fs2.writeFileSync)(path, content, options);
  try {
...
function cleanStaleTmp(path) {
  ...
  try {
    for (const entry of (0, import_node_fs2.readdirSync)(dir)) {
      ...
      if (Date.now() - (0, import_node_fs2.statSync)(full).mtimeMs > 10 * 60 * 1e3) {
...
function loadConfig() {
  ...
  const project = (0, import_node_fs2.existsSync)(PROJECT_CONFIG) ? readJSON(PROJECT_CONFIG) : null;
...
function removeProfile(name) {
  ...
  if ((0, import_node_fs2.existsSync)(PROJECT_CONFIG)) {
...
function setDefaultProfile(name) {
  ...
  if (project && project.default) {
    ...
    atomicWrite(PROJECT_CONFIG, JSON.stringify(project, null, 2) + "\n", "utf8");
    return { path: (0, import_node_path.resolve)(process.cwd(), PROJECT_CONFIG), scope: "\u9879\u76EE\u914D\u7F6E" };
  }
...
function netError(e, target) {
  if (isTimeout(e)) return `\u8BF7\u6C42\u8D85\u65F6\uFF08${TIMEOUT_MS / 1e3}s \u65E0\u54CD\u5E94\uFF09: ${target}`;
  const cause = e?.cause?.code || e?.cause?.message || "";
  return `\u65E0\u6CD5\u8FDE\u63A5 ${target}${cause ? `\uFF08${cause}\uFF09` : ""}`;
}

// src/auth.js
function profileVarPrefix(profileName) {
...
function resolveCredentials(profileName, cliOpts, cfg = {}) {
...
async function ensureSession(profileName, baseUrl, cliOpts) {
...
async function reLogin(profileName, baseUrl) {
  loadDotEnv();
...
// src/client.js
async function portalRequest(method, pathname, opts = {}) {
  ...
  async function doFetch(c) {
    ...
    try {
      return await fetchWithTimeout(url, {
        ...
        redirect: "manual",
        headers,
        body: body && method !== "GET" && method !== "DELETE" ? JSON.stringify(body) : void 0
      });
    } catch (e) {
...
async function portalJSON(method, pathname, opts = {}) {
...
async function getNamespaces(appId, portalEnv, cluster, opts) {
...
async function getItems(appId, portalEnv, cluster, ns, opts) {
  return portalJSON("GET", buildPath(appId, portalEnv, cluster, ns, "/items"), opts);
}
async function createItem(appId, portalEnv, cluster, ns, data, opts) {
  return portalJSON("POST", buildPath(appId, portalEnv, cluster, ns, "/item"), { ...opts, body: data });
}
async function updateItem(appId, portalEnv, cluster, ns, data, opts) {
  return portalJSON("PUT", buildPath(appId, portalEnv, cluster, ns, "/item"), { ...opts, body: data });
}
async function deleteItem(appId, portalEnv, cluster, ns, itemId, username, opts) {
...
async function publishRelease(appId, portalEnv, cluster, ns, model, opts) {
  const body = {
    appId,
    env: portalEnv,
    clusterName: cluster,
...
async function getActiveReleases(appId, portalEnv, cluster, ns, limit, opts) {
...
var FILE_FORMATS = /* @__PURE__ */ new Set(["yml", "yaml", "json"]);
...
function isAmbiguousNamespaceItems(items) {
  if (!Array.isArray(items)) return false;
  if (items.length === 0) return true;
  return items.length === 1 && items[0]?.key === CONTENT_KEY;
}
function pickNamespaceFormat(nsList2, nsName) {
  if (!Array.isArray(nsList2)) return null;
...
function parseFieldPath(path) {
  ...
  while (i < text.length) {
    ...
    if (ch === "[") {
      ...
      needSegment = false;
      const next = text[i];
      if (next !== void 0 && next !== "." && next !== "[") throw invalid(`\u4E0B\u6807\u540E\u4E0D\u80FD\u8DDF "${next}"`);
      continue;
    }
...
function parseFieldValue(raw, opts = {}) {
  if (opts.string) return raw;
  if (raw === "") return "";
  if (raw.includes("\n") || raw.includes("\r")) return raw;
  const doc = (0, import_yaml.parseDocument)(raw, { logLevel: "silent" });
...
function getField(content, format, path) {
...
function setField(content, format, path, value) {
  const fmt = normalizeFormat(format);
  const segments = Array.isArray(path) ? path : parseFieldPath(path);
  const pathLabel = Array.isArray(path) ? label(path) : String(path);
...
function renderFieldValue(value, format, opts = {}) {
...
function parseYamlDoc(content, format) {
  ...
  const docs = (0, import_yaml.parseAllDocuments)(text, { logLevel: "silent" });
  if (docs.length > 1) {
    throw new Error(`\u5185\u5BB9\u89E3\u6790\u5931\u8D25\uFF08\u683C\u5F0F ${fmt}\uFF09: \u4E0D\u652F\u6301\u591A\u6587\u6863\uFF08---\uFF09`);
  }
  const doc = docs[0] ?? (0, import_yaml.parseDocument)(text, { logLevel: "silent" });
  if (doc.errors.length > 0) {
...
function walkPlain(root, segments, pathLabel) {
  const at = (trail2) => trail2.length ? `\u8DEF\u5F84\u6BB5 "${label(trail2)}" ` : "\u6839\u8282\u70B9";
  let node = root;
  const trail = [];
  for (const seg of segments) {
    if (typeof seg === "number") {
      if (!Array.isArray(node)) {
        if (node === null || node === void 0) return { found: false };
...
function setPlain(root, segments, value, pathLabel) {
  const at = (trail2) => trail2.length ? `\u8DEF\u5F84\u6BB5 "${label(trail2)}" ` : "\u6839\u8282\u70B9";
  if (root === null || root === void 0) return buildSubtree(segments, value, pathLabel, []);
  let node = root;
  const trail = [];
  for (let k = 0; k < segments.length; k++) {
    ...
    if (typeof seg === "number") {
      ...
      const child2 = node[seg];
      if (child2 !== null && typeof child2 === "object") {
        trail.push(seg);
        node = child2;
        continue;
      }
      if (child2 === null || child2 === void 0) {
...
function setYamlValue(doc, segments, value, pathLabel) {
  const at = (trail2) => trail2.length ? `\u8DEF\u5F84\u6BB5 "${label(trail2)}" ` : "\u6839\u8282\u70B9";
  const needIndex = (trail2) => {
    const hint = trail2.length ? `"${label(trail2)}[0]"` : '"[0]"';
    return new Error(`\u5B57\u6BB5\u8DEF\u5F84 "${pathLabel}" \u65E0\u6CD5\u89E3\u6790: ${at(trail2)}\u662F\u6570\u7EC4\uFF0C\u8BF7\u4F7F\u7528\u4E0B\u6807\uFF08\u5982 ${hint}\uFF09`);
  };
  const noIndex = (trail2) => new Error(`\u5B57\u6BB5\u8DEF\u5F84 "${pathLabel}" \u65E0\u6CD5\u89E3\u6790: ${at(trail2)}\u662F\u5BF9\u8C61\uFF0C\u4E0D\u80FD\u4F7F\u7528\u4E0B\u6807`);
  const notContainer = (trail2) => new Error(`\u5B57\u6BB5\u8DEF\u5F84 "${pathLabel}" \u65E0\u6CD5\u89E3\u6790: ${at(trail2)}\u4E0D\u662F\u5BF9\u8C61\u6216\u6570\u7EC4`);
  if (doc.contents === null) {
    const node2 = doc.createNode(buildSubtree(segments, value, pathLabel, []));
    if (doc.commentBefore) {
      node2.commentBefore = doc.commentBefore;
  ...
  for (let k = 0; k < segments.length; k++) {
    ...
    if (typeof seg === "number") {
      if (!(0, import_yaml.isSeq)(node)) {
        if ((0, import_yaml.isMap)(node)) throw noIndex(trail);
      ...
      const child2 = node.items[seg];
      if ((0, import_yaml.isCollection)(child2)) {
        trail.push(seg);
        node = child2;
        continue;
      }
    ...
    if ((0, import_yaml.isSeq)(node)) throw needIndex(trail);
    if (!(0, import_yaml.isMap)(node)) throw notContainer(trail);
    if (last) {
      node.set(seg, value);
      return;
    }
    ...
    if ((0, import_yaml.isCollection)(child)) {
      trail.push(seg);
      node = child;
      continue;
    }
...
async function login2(profileArg, opts) {
  loadDotEnv();
...
function logout(profileArg, opts = {}) {
...
function profileAdd(name, vals) {
  ...
  if (vals.default) getAllProfiles();
  saveUserConfig({ profiles: { [name]: data } });
  let def = null;
  if (vals.default) def = setDefaultProfile(name);
  const text = `profile "${name}" \u5DF2\u6DFB\u52A0 (portal: ${portalEnv}, cluster: ${data.cluster})` + (def ? `
...
function profileDefault(name, opts = {}) {
  const { profiles } = getAllProfiles();
  if (!profiles[name]) {
    die(`profile "${name}" \u4E0D\u5B58\u5728`);
  }
...
async function nsList(appId, opts) {
  const ctx = profileCtx(null, opts);
  try {
    const cookie = await ensureSession(ctx.profileName, ctx.baseUrl, opts);
    const data = await getNamespaces(appId, ctx.portalEnv, ctx.cluster, { profileName: ctx.profileName, baseUrl: ctx.baseUrl, cookie });
    if (!Array.isArray(data) || data.length === 0) {
      if (opts.json) {
        output([], opts);
        return;
...
async function resolveFieldFormat(appId, ctx, items, cookie) {
  if (!isAmbiguousNamespaceItems(items)) return null;
  const nsList2 = await getNamespaces(appId, ctx.portalEnv, ctx.cluster, {
    profileName: ctx.profileName,
    baseUrl: ctx.baseUrl,
    cookie
  });
  const format = pickNamespaceFormat(nsList2, ctx.namespace);
  return isFileFormat(format) ? format : null;
}
async function configList(appId, opts) {
  const ctx = profileCtx(null, opts);
  try {
    const cookie = await ensureSession(ctx.profileName, ctx.baseUrl, opts);
    const data = await getItems(appId, ctx.portalEnv, ctx.cluster, ctx.namespace, { profileName: ctx.profileName, baseUrl: ctx.baseUrl, cookie });
    if (!Array.isArray(data) || data.length === 0) {
      if (opts.json) {
        output([], opts);
        return;
...
async function configGet(appId, key, opts) {
  const ctx = profileCtx(null, opts);
  try {
    const cookie = await ensureSession(ctx.profileName, ctx.baseUrl, opts);
    const data = await getItems(appId, ctx.portalEnv, ctx.cluster, ctx.namespace, { profileName: ctx.profileName, baseUrl: ctx.baseUrl, cookie });
    ...
    if (format) {
      const contentItem = items.find((i) => i.key === CONTENT_KEY);
      if (!contentItem) {
        die(`\u5B57\u6BB5 "${key}" \u4E0D\u5B58\u5728\uFF08\u914D\u7F6E\u9879 "${CONTENT_KEY}" \u5C1A\u672A\u521B\u5EFA\uFF09`);
      }
      const { found, value } = getField(contentItem.value, format, key);
      if (!found) die(`\u5B57\u6BB5 "${key}" \u4E0D\u5B58\u5728`);
      process.stdout.write(renderFieldValue(value, format, opts));
      return;
    }
...
async function configSet(appId, key, value, opts) {
  const ctx = profileCtx(null, opts);
  try {
    const cookie = await ensureSession(ctx.profileName, ctx.baseUrl, opts);
    const data = await getItems(appId, ctx.portalEnv, ctx.cluster, ctx.namespace, { profileName: ctx.profileName, baseUrl: ctx.baseUrl, cookie });
    ...
    const format = await resolveFieldFormat(appId, ctx, items, cookie);
    if (format) {
      const existing2 = items.find((i) => i.key === CONTENT_KEY);
      const parsed = parseFieldValue(value, { string: !!opts.string });
      const prior = getField(existing2?.value ?? "", format, key);
      const next = setField(existing2?.value ?? "", format, key, parsed);
      const action = prior.found ? "update" : "create";
      const base2 = { action, key, namespace: ctx.namespace, value: parsed, needsPublish: true };
      if (opts["dry-run"]) {
        if (existing2) {
          emit(
            opts,
          ...
          emit(
            opts,
            { dryRun: true, ...base2, contentItemCreated: true },
            `[dry-run] \u5C06\u65B0\u589E\u5B57\u6BB5 "${key}"\uFF08\u5C06\u521B\u5EFA\u914D\u7F6E\u9879 "${CONTENT_KEY}"\uFF0C\u672A\u6267\u884C\uFF1B\u751F\u6548\u9700 config publish\uFF09`
          );
        }
        return;
      }
      if (existing2) {
        await updateItem(appId, ctx.portalEnv, ctx.cluster, ctx.namespace, {
          id: existing2.id,
          key: CONTENT_KEY,
          value: next,
          comment: opts.comment || existing2.comment || "",
          dataChangeLastModifiedBy: username,
          dataChangeLastModifiedTime: (/* @__PURE__ */ new Date()).toISOString()
        }, { profileName: ctx.profileName, baseUrl: ctx.baseUrl, cookie });
        emit(opts, base2, `\u5B57\u6BB5 "${key}" \u5DF2${prior.found ? "\u66F4\u65B0" : "\u65B0\u589E"}\uFF08\u9700 config publish \u624D\u751F\u6548\uFF09`);
      } else {
        await createItem(appId, ctx.portalEnv, ctx.cluster, ctx.namespace, {
          key: CONTENT_KEY,
          value: next,
        ...
        emit(
          opts,
          { ...base2, contentItemCreated: true },
          `\u5B57\u6BB5 "${key}" \u5DF2\u65B0\u589E\uFF08\u5DF2\u521B\u5EFA\u914D\u7F6E\u9879 "${CONTENT_KEY}"\uFF0C\u9700 config publish \u624D\u751F\u6548\uFF09`
        );
      }
      return;
    }
...
async function configRm(appId, key, opts) {
  const ctx = profileCtx(null, opts);
  try {
    const cookie = await ensureSession(ctx.profileName, ctx.baseUrl, opts);
    const data = await getItems(appId, ctx.portalEnv, ctx.cluster, ctx.namespace, { profileName: ctx.profileName, baseUrl: ctx.baseUrl, cookie });
    ...
    const session = loadSession()[ctx.profileName];
    await deleteItem(appId, ctx.portalEnv, ctx.cluster, ctx.namespace, item.id, session?.username || "", { profileName: ctx.profileName, baseUrl: ctx.baseUrl, cookie });
    emit(opts, base, `\u914D\u7F6E\u9879 "${key}" \u5DF2\u5220\u9664\uFF08\u9700 config publish \u624D\u751F\u6548\uFF09`);
  } catch (e) {
    fatal(e);
  }
}
async function configPublish(appId, opts) {
  const ctx = profileCtx(null, opts);
  ...
  if (opts["dry-run"]) {
    const detail = `title: ${title}` + (opts.emergency ? "\uFF0C\u7D27\u6025\u53D1\u5E03" : "") + (opts.comment ? `\uFF0Ccomment: ${opts.comment}` : "");
    emit(opts, {
      dryRun: true,
      action: "publish",
  ...
  try {
    ...
    const session = loadSession()[ctx.profileName];
    const username = session?.username || "";
    const result = await publishRelease(appId, ctx.portalEnv, ctx.cluster, ctx.namespace, {
      title,
      comment: opts.comment || "",
      releasedBy: username,
      emergency: opts.emergency || false
    }, { profileName: ctx.profileName, baseUrl: ctx.baseUrl, cookie });
    if (result && result.id !== void 0) {
      const t = result.releaseTitle ? `, title: ${result.releaseTitle}` : "";
      emit(
        opts,
        { action: "publish", namespace: ctx.namespace, releaseId: result.id, title: result.releaseTitle || title },
...
async function configReleases(appId, opts) {
  const ctx = profileCtx(null, opts);
  const limit = opts.limit || 10;
  try {
    const cookie = await ensureSession(ctx.profileName, ctx.baseUrl, opts);
    const data = await getActiveReleases(appId, ctx.portalEnv, ctx.cluster, ctx.namespace, limit, { profileName: ctx.profileName, baseUrl: ctx.baseUrl, cookie });
...
function promptConfirm(question) {
  ...
  return new Promise((resolve2) => {
    ...
    rl.once("line", (line) => {
      answered = true;
      rl.close();
      resolve2(/^y(es)?$/i.test(line.trim()));
    });
    rl.once("SIGINT", () => {
      answered = true;
      rl.close();
...
function pos(parsed, n, usage) {
  if (parsed.positionals.length < n) die2(`\u7F3A\u5C11\u53C2\u6570\u3002\u7528\u6CD5: ${usage}`);
...
var GLOBAL_VALUE_OPTS = /* @__PURE__ */ new Set(["-p", "--profile", "-n", "--namespace", "--cluster"]);
var STOP_TOKENS = /* @__PURE__ */ new Set(["-h", "--help", "-V", "--version", "--"]);
...
`);
  ...
  switch (sub) {
    ...
    default:
      die2(`\u672A\u77E5 profile \u5B50\u547D\u4EE4: "${sub}"\u3002\u53EF\u7528: list, add, rm, default`);
  }
}
...
`);
  ...
  switch (sub) {
    ...
    case "releases": {
      ...
      return configReleases(p.positionals[0], { ...p.values, limit });
    }
    default:
      die2(`\u672A\u77E5 config \u5B50\u547D\u4EE4: "${sub}"\u3002\u53EF\u7528: ls, get, set, rm, publish, releases`);
  }
}
