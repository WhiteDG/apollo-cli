---
name: apollo
description: 用本技能自带的 apollo-cli 单文件产物（无需预装、无需 npm 安装）查看、修改、删除、发布 Apollo 配置中心的配置。当用户提到 Apollo、配置中心、命名空间、appId、发布配置、发布历史，或要查看/修改某个配置项（超时、开关、连接串、yml/yaml/json 文件里的字段等）时使用本技能。即使用户没有提到 apollo-cli 这个名字，只要意图是读写 Apollo 配置（如"把 fat 环境的 timeout 改成 5000"、"看下 app.yml 里的 server.port"、"发布一下配置"），都应使用本技能，而不是手写 HTTP 请求或让用户去 Portal 网页操作。
---

# apollo-cli 操作指南（CLI 随技能自带）

本技能自带打包好的 apollo-cli 单文件产物 `scripts/apollo-cli.cjs`：机器上有 Node.js >= 21 就能直接运行，**不需要预先安装任何 CLI**。不要 `npm i -g apollo-cli`（npm registry 上的同名包是另一个工具），也不要 `npm i -g github:WhiteDG/apollo-cli#master`。它通过 Portal 账号密码登录（cookie 会话，过期自动重登），不需要 OpenAPI token。凡是读写 Apollo 配置的任务，一律用它完成，不要自己拼 Portal 的 HTTP 接口。

## 执行方式：先解析 <SK>

`<SK>` = 本次加载的 SKILL.md 所在目录的**绝对路径**。SKILL.md 里没有 `${SKILL_ROOT}` 这类可用变量，必须自行解析；解析一次后在本次会话内复用同一个值：

1. 加载上下文已给出本 skill 目录（形如 `.../skills/apollo`）时，直接用它（符号链接路径也能用）。
2. 否则按标准安装位置探测（命中即止）：`ls -d "$HOME"/.qoder/skills/apollo "$HOME"/.agents/skills/apollo 2>/dev/null`。
3. 仍找不到就用 Glob 搜 `**/skills/apollo/scripts/apollo-cli.cjs`，或直接问用户 skill 装在哪。

之后所有命令统一为 `node <SK>/scripts/apollo-cli.cjs <命令> [选项]`（把 `<SK>` 换成解析出的绝对路径）。

- **用绝对路径调用，且不要 `cd` 到 skill 目录**：profile 与凭据按当前工作目录解析（见下文"命令要在项目根目录执行"），命令必须留在项目根目录执行。
- 不要用 PATH 上可能存在的全局 `apollo-cli`，一律走自带产物，避免版本漂移。
...
- 报 `node: command not found`，或 `node --version` 低于 21 时，告知用户"本技能自带的 CLI 需要 Node.js >= 21，请先安装/升级 Node"，不要建议安装 CLI，也不要绕行。
...
下面 9 条命令都省略了前缀，实际执行统一是 `node <SK>/scripts/apollo-cli.cjs ...`：
